...
function moveSnake() {
  ...
  */
  ...
  for (let i = snake.body.length - 1; i > 0; i--) {
    // currentSnakeSquare refers to the segment being moved in the current iteration (e.g., the tail, then the segment before the tail, etc.)
    var currentSnakeSquare = snake.body[i];
    // snakeSquareInFront refers to the segment immediately ahead of the current segment
    var snakeSquareInFront = snake.body[i - 1];

    // This function makes the current square adopt the position of the square in front of it
    moveBodyAToBodyB(currentSnakeSquare, snakeSquareInFront);

    // This function likely updates the physical position/rendering of the square on the screen
